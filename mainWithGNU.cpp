...
int mainWithGNU() {
    ...
    cout << "x~:" << endl << x;
    fprintf(pipe, "plot [25 : 50] [0 : 8] %lf*x**3 + %lf*x**2 + %lf*x**1 + %lf*x**0 , '-' using 1:2 with points\n",
            x.data[3][0], x.data[2][0], x.data[1][0], x.data[0][0]);
    for (int i = 0; i < m; i++) {
        fprintf(pipe, "%f\t%f\n", data.data[i][0], data.data[i][1]);
        cout << data.data[i][0] << " " << data.data[i][1] << endl;
    }
    fprintf(pipe, "e\n");
    fflush(pipe);
#ifdef WIN32
